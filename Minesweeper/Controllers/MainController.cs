    ...
    public static class MainController {
            ...
            if (button.IsActive && !IsFirstStep) {
                ...
                switch (button.CurrentPircture) {
                    case 0:
                        ...
                        if (Database.question == "yes") button.CurrentPircture++;
                        else button.CurrentPircture = 2;
        ...
        private static void LeftButtonPressed(MyButton button) {
            if (button.IsActive && !button.IsFlagCheck) {
                Database.play.Stop();
                if (IsFirstStep) {
                    Database.ChangeSound("sounds/firstclick.wav");
                    IsFirstStep = false;
                    GenerateField(button);
                }
                else Database.ChangeSound("sounds/fieldclick.wav");
